...
nb2 = {
    ...
    "cells": [
        ...
        # ── 2. Clone + deps ──
        {
            ...
            "outputs": [],
            "source": [
                "# @title 2. Clone Wan 2.2 repo + install animation dependencies\n",
                "%cd /content\n",
                "![ -d Wan2.2 ] || git clone --depth 1 https://github.com/Wan-Video/Wan2.2.git\n",
                "%cd /content/Wan2.2\n",
                "!pip install -q -e .\n",
                "!pip install -q -r requirements.txt\n",
                "# Shared deps + animation specific (including onnxruntime-gpu for face/pose)\n",
                "!pip install -q ninja iopath ftfy regex decord loguru 'huggingface_hub[cli]'\n",
                "!pip install -q onnxruntime-gpu peft pandas matplotlib sentencepiece\n",
                "# SAM-2 (Facebook's Segment Anything) - clone, install and link configs\n",
                "%cd /content\n",
                "![ -d sam2 ] || git clone https://github.com/facebookresearch/sam2.git\n",
                "%cd /content/sam2\n",
                "!git checkout 0e78a118995e66bb27d78518c4bd9a3e95b4e266 -q\n",
                "!pip install -q . --no-build-isolation\n",
                "# --- HYDRA CONFIG FIX ---\n",
                "# Force-copy configs to site-packages so Hydra's pkg:// provider finds them\n",
                "import site, shutil\n",
                "sp = site.getsitepackages()[0]\n",
                "print(f'Injecting SAM2 configs to {sp}/sam2_configs')\n",
                "shutil.copytree('/content/sam2/sam2_configs', f'{sp}/sam2_configs', dirs_exist_ok=True)\n",
                "# Also link to local directory for extra safety\n",
                "!ln -sfn /content/sam2/sam2_configs /content/Wan2.2/sam2_configs\n",
                "%cd /content/Wan2.2\n",
                "print('Setup done. All animation dependencies installed.')\n",
            ],
        },
        # ── 3. Download weights (shared on Drive) ──
        {
        ...
        # ── 4. Select inputs & settings ──
        {
            ...
            "outputs": [],
            "source": [
                "# @title 4. Select inputs & settings\n",
                ...
                "# @markdown ### Duration (frame count must be 4k+1, rendered at 16fps)\n",
                'FRAME_NUM = 81  # @param [33, 49, 65, 81, 97, 113, 129] {type:"raw"}\n',
                'print(f"Duration: ~{FRAME_NUM/16:.1f}s ({FRAME_NUM} frames @ 16fps)  SEED={SEED}")\n',
            ],
        },
        # ── 5. Preprocess ──
        {
        ...
        # ── 6. Inference ──
        {
            ...
            "source": [
                ...
                'flags = "--offload_model True --convert_model_dtype --t5_cpu"\n',
                "cmd = (\n",
                "    f'python generate.py --task animate-14B '\n",
                '    f\'--ckpt_dir "{CKPT_DIR}" --src_root_path "{PROC_DIR}/" --refert_num 1 \'\n',
                '    f"--replace_flag --use_relighting_lora --base_seed {SEED} --frame_num {FRAME_NUM} {flags}"\n',
                ")\n",
                'print("Running:\\n", cmd, "\\n")\n',
                "t0 = time.time()\n",
                "!{cmd}\n",
